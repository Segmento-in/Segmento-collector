...
def _get_active_destination(uid: str) -> dict | None:
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT dest_type, host, port, username, password, database_name, format FROM destination_configs WHERE uid=? AND source=? AND is_active=1 LIMIT 1", (uid, SOURCE))
    row = fetchone_secure(cur)
    con.close()
